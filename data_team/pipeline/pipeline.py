...
def _extract_indexes(model_files: list[Path]) -> dict[str, list[str]]:
    ...
    for path in model_files:
        ...
        for match in re.finditer(r'Index\(\s*["\']([^"\']+)["\']', source):
            ...
            tablename_match = re.search(r'__tablename__\s*=\s*["\']([^"\']+)["\']', source)
            if tablename_match:
...
def _check_usage_log_actions() -> tuple[set[str], set[str]]:
    ...
    expected = {
        "csv_upload", "search_create", "smart_search",
        "intro_draft", "intro_request", "marketplace_search",
        "application_create", "contacts_list", "contact_delete",
    }
...
def _check_index_coverage(
    ...
    analytics_tables = {"usage_logs", "csv_uploads", "search_requests", "applications",
                        "match_results", "credit_transactions", "intro_facilitations"}
