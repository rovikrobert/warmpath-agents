...
_MESSAGES_CREATE_RE = re.compile(
    r"client\.messages\.(create|stream)", re.MULTILINE
)
        ...
        for match in _MESSAGES_CREATE_RE.finditer(source):
            ...
            if has_system:
                ...
                for sp_match in _SYSTEM_ASSIGN_RE.finditer(source):
                    ...
                    sp_content_start = source.index(
                        '"""', sp_start
                    )
                    try:
                        sp_content_end = source.index(
                            '"""', sp_content_start + 3
                        )
...
def _scan_missing_indexes(
    ...
    queried: dict[tuple[str, str], list[tuple[str, int]]] = {}  # (model, col) -> [(file, line)]
    ...
    for (model, col), locations in queried.items():
        ...
        if model.islower() or model in ("func", "select", "delete", "update", "and_", "or_"):
            continue
...
def _scan_table_growth(models_dir: Path) -> tuple[list[Finding], dict[str, Any]]:
    ...
    for path in retention_files:
        ...
        if "usage_log" in source.lower() and (
            "delete" in source.lower() or "retention" in source.lower()
            or "cleanup" in source.lower() or "purge" in source.lower()
        ):
...
def scan() -> AgentReport:
    ...
    try:
        ...
        learning.record_scan(AGENT_NAME, {
            ...
            "unindexed_query_columns": all_metrics.get("unindexed_query_columns", 0),
