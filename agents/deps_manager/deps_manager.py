...
from agents.shared.config import PROJECT_ROOT, DEP_STALE_DAYS, SKIP_DIRS
...
_STDLIB_MODULES: set[str] = {
    "abc", "asyncio", "base64", "binascii", "builtins", "calendar",
    "collections", "concurrent", "configparser", "contextlib", "copy",
    "csv", "ctypes", "dataclasses", "datetime", "decimal", "difflib",
    "email", "enum", "errno", "fnmatch", "functools", "gc", "getpass",
    "glob", "gzip", "hashlib", "hmac", "html", "http", "importlib",
    ...
    "time", "timeit", "traceback", "types", "typing", "unicodedata",
...
def scan() -> AgentReport:
    ...
    pinned_count = sum(1 for d in deps if d["version_op"] in ("==", ">=", "<=", "~=", ">", "<"))
    ...
    cve_count = sum(1 for f in audit_findings if f.category == "dependency-vulnerability")
