"""Security agent — wraps scripts/security_scan.py to output AgentReport."""
...
def _convert_findings(raw: list[dict]) -> list[Finding]:
    """Convert the scripts/security_scan.py findings dicts to Finding objects."""
    ...
    for i, f in enumerate(raw):
        ...
        base_id = f"SEC-{category.upper().replace('_', '-')}"
...
def scan() -> AgentReport:
    """Run the security scanner and return an AgentReport."""
    start = time.time()

    # Import the script module and reset its state
    import scripts.security_scan as sec

    sec.findings.clear()

    # Run all scan phases, capturing output
    try:
        sec.scan_dependencies()
    except Exception as exc:
        logger.warning("Dependency scan failed: %s", exc)

    try:
        sec.scan_code_patterns()
    except Exception as exc:
        logger.warning("Code pattern scan failed: %s", exc)

    try:
        sec.scan_config()
    except Exception as exc:
        logger.warning("Config scan failed: %s", exc)

    try:
        sec.scan_auth_coverage()
    except Exception as exc:
        logger.warning("Auth coverage scan failed: %s", exc)

    try:
        sec.scan_input_validation()
    except Exception as exc:
        logger.warning("Input validation scan failed: %s", exc)

    # Convert findings
    findings = _convert_findings(sec.findings)
    ...
    logger.info("Security scan complete: %d findings in %.1fs", len(findings), elapsed)
    return report
