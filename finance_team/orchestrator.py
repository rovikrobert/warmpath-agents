"""Finance team orchestrator — CLI entry point for running finance agents.

Usage:
    python -m finance_team.orchestrator --all               # Run all agents
    python -m finance_team.orchestrator --agent finance_manager  # Run single agent
    python -m finance_team.orchestrator --daily-brief        # Brief from cached reports
    python -m finance_team.orchestrator --weekly             # Weekly deep dive
    python -m finance_team.orchestrator --monthly            # Monthly review
    python -m finance_team.orchestrator --intel              # Intel freshness check
    python -m finance_team.orchestrator --intel-report       # Full intel summary
...
def cmd_all() -> None:
    """Run all finance agents sequentially, then produce daily brief."""
    print("=" * 60)
    print("FINANCE TEAM — Full Scan")
    print("=" * 60)
    ...
    agent_order = ["finance_manager", "credits_manager", "investor_relations", "legal_compliance", "finance_lead"]
    ...
    # Print daily brief
    from finance_team.finance_lead.finance_lead import generate_daily_brief
...
def cmd_weekly() -> None:
    ...
    from finance_team.finance_lead.finance_lead import generate_weekly_report
    print(generate_weekly_report())


def cmd_monthly() -> None:
    """Generate monthly review."""
    from finance_team.finance_lead.finance_lead import generate_monthly_review
    print(generate_monthly_review())


def cmd_intel() -> None:
    """Check finance intelligence freshness."""
    from finance_team.shared.intelligence import FinanceIntelligence
    fi = FinanceIntelligence()
    freshness = fi.check_freshness()
    agenda = fi.generate_research_agenda()
...
def cmd_intel_report() -> None:
    ...
    report = fi.generate_intel_report()

    print("=" * 60)
    print("FINANCE TEAM — Intelligence Report")
    print("=" * 60)
...
def cmd_learning_report() -> None:
    ...
    print("=" * 60)
    print("FINANCE TEAM — Learning Reports")
    print("=" * 60)

    for agent_name in FINANCE_AGENT_NAMES:
...
def cmd_research_agenda() -> None:
    ...
    print("=" * 60)
    print("FINANCE TEAM — Research Agenda")
    print("=" * 60)
...
def cmd_compliance_audit() -> None:
    """Run compliance-focused audit from cached reports."""
    from finance_team.finance_lead.finance_lead import _load_latest_reports

    reports = _load_latest_reports()

    print("=" * 60)
    print("FINANCE TEAM — Compliance Audit")
    print("=" * 60)

    if not reports:
        print("\nNo reports available. Run --all first.")
        return
    ...
    if not all_compliance:
        print("\nNo compliance findings. All checks passing.")
        return

    by_reg: dict[str, list] = {}
    for c in all_compliance:
        reg = c.regulation or "General"
        if reg not in by_reg:
            by_reg[reg] = []
        by_reg[reg].append(c)

    for reg, findings in sorted(by_reg.items()):
        print(f"\n## {reg} ({len(findings)} findings)\n")
        for c in findings:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="WarmPath Finance Team Orchestrator"
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--all", action="store_true", help="Run all finance agents")
    ...
    group.add_argument("--compliance-audit", action="store_true", help="Compliance-focused report")
