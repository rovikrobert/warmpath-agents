...
def _check_earn_spend_rules(
    ...
    total_expected = len(EXPECTED_CREDIT_EARN_ACTIONS) + len(
        EXPECTED_CREDIT_SPEND_ACTIONS
    )
...
def _check_balance_integrity(
    ...
    has_sum_query = bool(
        re.search(r"\bSUM\b|func\.sum|sum\(", credits_svc_source, re.IGNORECASE)
    )
...
def _check_abuse_prevention(
    ...
    negative_balance_patterns = [
        re.compile(
            r"insufficient|negative|balance\s*[<=>]+\s*0|balance\s*<=\s*0",
            re.IGNORECASE,
        ),
    ]
    ...
    bulk_cap_patterns = [
        re.compile(
            r"max.*earn|earn.*max|earn.*cap|cap.*earn|MAX_EARN|bulk.*limit",
            re.IGNORECASE,
        ),
    ]
    ...
    metrics["abuse_prevention_score"] = round((3 - len(missing_safeguards)) / 3, 3)
