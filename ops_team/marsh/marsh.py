...
def _check_marketplace_model(
    ...
    if listing_missing:
        severity = "high" if any(
            ...
            for f in ("company_id", "role_level", "is_available", "network_holder_id")
        ) else "medium"
        ...
        mkt_findings.append(MarketplaceFinding(
            id="marsh-mkt-001",
            category="model_completeness",
            severity=severity,
    ...
    if intro_missing:
        severity = "high" if any(
            f in intro_missing
            for f in ("status", "job_seeker_id", "network_holder_id", "marketplace_listing_id")
        ) else "medium"
        findings.append(Finding(
            id="marsh-002",
            severity=severity,
...
_EARN_ACTIONS = ["csv_upload", "intro_facilitation", "data_freshness", "welcome_bonus", "purchase"]
_SPEND_ACTIONS = ["cross_network_search", "marketplace_search", "request_intro", "intro_request"]
...
def _check_intro_pipeline(
    ...
    has_request_intro = bool(re.search(r'request.intro|request_intro', marketplace_api_source))
    ...
    if statuses_missing:
        severity = "high" if any(s in statuses_missing for s in ("requested", "approved")) else "medium"
        findings.append(Finding(
            id="marsh-023",
            severity=severity,
            category="intro_pipeline",
    ...
    has_usage_log = "UsageLog" in marketplace_api_source or "usage_log" in marketplace_api_source
...
def _check_marketplace_actions(
    ...
    _ACTION_ALIASES: dict[str, list[str]] = {
        "cross_network_search": ["cross_network_search", "marketplace_search", "marketplace.search"],
...
def _check_suppression_impact(
    ...
    has_suppression_model = "suppression_list" in privacy_source or "SuppressionList" in privacy_source
    ...
    has_suppression_check_api = (
        "suppression" in api_source.lower()
        or "hash_for_suppression" in api_source
    )
...
def scan() -> OpsTeamReport:
    ...
    metrics["files_scanned"] = sum(
        1 for s in [model_source, credit_source, marketplace_api_source, credits_api_source] if s
    )
    ...
    _check_credit_economy(
        credit_source, credits_api_source, findings, mkt_findings, metrics,
    ...
    _check_suppression_impact(model_source, marketplace_api_source, findings, mkt_findings, metrics)
    ...
    # Track KPIs
    ls.track_kpi("marketplace_action_coverage", metrics.get("marketplace_action_coverage", 0))
