"""Ops team orchestrator — CLI entry point for running operations agents.

Usage:
    python -m ops_team.orchestrator --all               # Run all agents
    python -m ops_team.orchestrator --agent keevs        # Run single agent
    python -m ops_team.orchestrator --daily-brief        # Brief from cached reports
    python -m ops_team.orchestrator --weekly             # Weekly deep dive
    python -m ops_team.orchestrator --monthly            # Monthly review
    python -m ops_team.orchestrator --intel              # Intel freshness check
...
from ops_team.shared.config import OPS_AGENT_NAMES  # noqa: E402
...
def cmd_all() -> None:
    """Run all ops agents sequentially, then produce daily brief."""
    print("=" * 60)
    print("OPS TEAM — Full Scan")
    print("=" * 60)
    ...
    agent_order = ["keevs", "treb", "naiv", "marsh", "ops_lead"]
...
def cmd_intel() -> None:
    """Check ops intelligence freshness."""
...
def cmd_intel_report() -> None:
    ...
    print("=" * 60)
    print("OPS TEAM — Intelligence Report")
    print("=" * 60)
...
def cmd_learning_report() -> None:
    ...
    print("=" * 60)
    print("OPS TEAM — Learning Reports")
    print("=" * 60)

    for agent_name in OPS_AGENT_NAMES:
...
def cmd_research_agenda() -> None:
    ...
    print("=" * 60)
    print("OPS TEAM — Research Agenda")
    print("=" * 60)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="WarmPath Ops Team Orchestrator")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--all", action="store_true", help="Run all ops agents")
