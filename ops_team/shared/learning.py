"""Self-learning system for ops team agents.

Provides `OpsLearningState` — a full learning state tracker with:
...
Same API pattern as product_team/shared/learning.py but stores state in
ops_team/{agent}/state.json to keep the teams independent.
"""
...
def _state_path(agent: str) -> Path:
    return OPS_TEAM_DIR / agent / "state.json"
...
# ---------------------------------------------------------------------------
# OpsLearningState class
# ---------------------------------------------------------------------------


class OpsLearningState:
    """Full learning state for an ops team agent with advanced analytics."""
    ...
    def record_health_snapshot(
        ...
        """Record a point-in-time ops health score."""
...
def get_learning_state(agent: str) -> OpsLearningState:
    """Return a fully loaded OpsLearningState instance."""
    return OpsLearningState(agent)
