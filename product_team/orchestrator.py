"""Product team orchestrator — CLI entry point for running product agents.

Usage:
    python -m product_team.orchestrator --all               # Run all agents
    python -m product_team.orchestrator --agent ux_lead     # Run single agent
    python -m product_team.orchestrator --daily-brief       # Brief from cached reports
    python -m product_team.orchestrator --weekly            # Weekly deep dive
    python -m product_team.orchestrator --monthly           # Monthly review
    python -m product_team.orchestrator --intel             # Intel freshness check
    python -m product_team.orchestrator --intel-report      # Full intel summary
    python -m product_team.orchestrator --learning-report   # Meta-learning reports
    python -m product_team.orchestrator --research-agenda   # Research priorities
    python -m product_team.orchestrator --consult "How should we improve onboarding?"
"""
...
from product_team.shared.config import PRODUCT_AGENT_NAMES  # noqa: E402
...
def _run_agent(name: str) -> object | None:
    ...
    try:
        ...
        try:
            ...
            publish_report("product_team", report.agent, report.serialize())
        except Exception:
...
def cmd_all() -> None:
    """Run all product agents sequentially, then produce daily brief."""
    print("=" * 60)
    print("PRODUCT TEAM — Full Scan")
    print("=" * 60)
...
def cmd_intel_report() -> None:
    ...
    print("=" * 60)
    print("PRODUCT TEAM — Intelligence Report")
    print("=" * 60)
...
def cmd_learning_report() -> None:
    ...
    print("=" * 60)
    print("PRODUCT TEAM — Learning Reports")
    print("=" * 60)

    for agent_name in PRODUCT_AGENT_NAMES:
...
def cmd_research_agenda() -> None:
    ...
    print("=" * 60)
    print("PRODUCT TEAM — Research Agenda")
    print("=" * 60)
