...
def _map_user_journeys(
    ...
    for persona_id, persona in PERSONAS.items():
        ...
        insights.append(ProductInsight(
            ...
            recommendation="Fill journey gaps" if coverage < 1.0 else "Journey complete",
            confidence=0.9,
            persona=persona_id,
        ))
...
def _check_persona_balance(
    ...
    insights.append(ProductInsight(
        ...
        impact="Balanced product investment" if 0.6 <= balance <= 1.5 else "Potential imbalance in persona investment",
        recommendation="Review supply-side features" if balance < 0.6 else "Balance is acceptable",
        confidence=0.8,
    ))
...
def _catalog_research_sources(
    ...
    researcher_categories = [
        cat for cat, meta in PRODUCT_INTEL_CATEGORIES.items()
...
def _scan_page_content(
    ...
    api_pattern = re.compile(r'(?:fetch|axios|api\.|useQuery|useMutation)', re.IGNORECASE)
...
def scan() -> ProductTeamReport:
    ...
    learning_updates = [f"Scanned {len(page_files)} page files, {len(insights)} insights"]
