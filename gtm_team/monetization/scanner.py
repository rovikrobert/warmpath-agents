...
def _check_pricing_architecture(
    ...
    if API_DIR.is_dir():
        for py_file in sorted(API_DIR.glob("*.py")):
            ...
            if re.search(
                r"stripe|payment|checkout|subscription|billing", content, re.IGNORECASE
            ):
                stripe_found = True
    ...
    insights.append(
        MarketInsight(
            ...
            recommended_response="Complete pricing-to-implementation mapping"
            if not stripe_found
            else "Maintain pricing alignment",
            urgency="this_week"
            if tiers_found == 0 and not stripe_found
            else "this_month",
            confidence="high",
...
def _check_credit_economy(
    ...
    credit_signals = {
        "earn_action": bool(
            re.search(
                r"earn|add_credits|credit_earn|award", credits_content, re.IGNORECASE
            )
        ),
        "spend_action": bool(
            re.search(
                r"spend|deduct|charge|consume|subtract", credits_content, re.IGNORECASE
            )
        ),
        "expiry": bool(
            re.search(
                r"expir|ttl|stale|month|valid_until", credits_content, re.IGNORECASE
            )
        ),
        "non_transferable": bool(
            re.search(r"transfer|non.transfer|user_id", credits_content, re.IGNORECASE)
        ),
        "balance_check": bool(
            re.search(
                r"balance|sum|total|sufficient|enough", credits_content, re.IGNORECASE
            )
        ),
    }
    ...
    insights.append(
        MarketInsight(
            ...
            evidence=", ".join(
                f"{k}={'yes' if v else 'no'}" for k, v in credit_signals.items()
            ),
            strategic_impact="Credit economy drives marketplace liquidity and retention",
            recommended_response="Close credit economy gaps"
            if missing
            else "Credit economy is solid",
...
def _check_feature_gating(
    ...
    gating_patterns = [
        ...
        re.compile(
            r"is_subscribed|has_subscription|check_plan|require_plan", re.IGNORECASE
        ),
        re.compile(r"feature_flag|feature_gate|access_level|permission", re.IGNORECASE),
        re.compile(
            r"credit.*check|check.*credit|sufficient.*credit|enough.*credit",
            re.IGNORECASE,
        ),
    ]
    ...
    insights.append(
        MarketInsight(
            ...
            recommended_response="Implement gating"
            if not gated_files
            else "Review gating completeness",
...
def _check_revenue_model(
    ...
    revenue_signals = {
        "revenue_model": bool(
            re.search(r"revenue\s+model|business\s+model|monetiz", all_lower)
        ),
        "pricing_detail": bool(
            re.search(r"\$\d+.*month|\$\d+.*year|per\s+month", all_lower)
        ),
        "unit_economics": bool(
            re.search(
                r"ltv|cac|arpu|churn|conversion\s+rate|unit\s+economics", all_lower
            )
        ),
        "projection": bool(
            re.search(r"projection|forecast|runway|revenue\s+target|mrr|arr", all_lower)
        ),
    }
    ...
    insights.append(
        MarketInsight(
            ...
            evidence=", ".join(
                f"{k}={'yes' if v else 'no'}" for k, v in revenue_signals.items()
            ),
            strategic_impact="Revenue model clarity drives fundraising and operational decisions",
            recommended_response="Complete revenue model"
            if found < total
            else "Revenue model documented",
...
def _check_marketplace_monetization(
    ...
    monetization_hooks = {
        "credit_deduction": bool(
            re.search(r"credit|deduct|charge|spend|cost", combined, re.IGNORECASE)
        ),
        "billing_trigger": bool(
            re.search(r"bill|invoice|payment|stripe|checkout", combined, re.IGNORECASE)
        ),
        "intro_cost": bool(
            re.search(
    ...
    insights.append(
        MarketInsight(
            ...
            evidence=", ".join(
                f"{k}={'yes' if v else 'no'}" for k, v in monetization_hooks.items()
            ),
            strategic_impact="Marketplace monetization hooks enforce the credit economy",
            recommended_response="Close monetization gaps"
            if missing
            else "Hooks complete",
            urgency="this_month" if found < 2 else "monitor",
            confidence="high",
...
def _compute_monetization_readiness(metrics: dict[str, Any]) -> float:
    ...
    pricing_score = 50.0 * min(
        1.0, metrics.get("pricing_tiers_documented", 0) / 2
...
if __name__ == "__main__":
    ...
    print(
        f"Monetization readiness: {report.metrics.get('monetization_readiness_score', 0)}/100"
    )
