"""Self-learning system for GTM team agents.

Provides `GTMLearningState` — a full learning state tracker with:
...
gtm_team/{agent}/state.json to keep the teams independent.
"""
...
def _state_path(agent: str) -> Path:
    return GTM_TEAM_DIR / agent / "state.json"
...
# ---------------------------------------------------------------------------
# GTMLearningState class
# ---------------------------------------------------------------------------


class GTMLearningState:
    """Full learning state for a GTM team agent with advanced analytics."""
    ...
    def record_health_snapshot(
        ...
        """Record a point-in-time GTM health score."""
...
def get_learning_state(agent: str) -> GTMLearningState:
    """Return a fully loaded GTMLearningState instance."""
    return GTMLearningState(agent)
